    ...
    def __init__(self, csv_file):
        ...
        for i in range(1, self.motion_data_np.shape[0]):
            ...
            if self.motion_data_np[i, 3] != _id:
                icp_poses[_id, :2] = self.motion_data_np[i, 7:9]
                icp_poses[_id, 2:] = quaternion_to_euler(self.motion_data_np[i, 13], self.motion_data_np[i, 10],
                                                       self.motion_data_np[i, 11], self.motion_data_np[i, 12])[2]
                self.data_x[_id, 0] = icp_poses[_id, 2] # roll
                # we compute velocity
                ...
                self.data_x[_id, 1] = v_body[0] # body v_x
                self.data_x[_id, 2] = v_body[1] # body v_y
                self.data_x[_id, 3] = (icp_poses[_id, 4] - icp_poses[_id - 1, 4]) / dt  # yaw rate, no need for transform
                self.data_x[_id, 4] = self.motion_data_np[i - 1, 4]  # u_left_t0
                self.data_x[_id, 5] = self.motion_data_np[i - 1, 6]  # u_right_t0
                self.data_x[_id, 6] = self.motion_data_np[i, 4]  # u_left_t1
                self.data_x[_id, 7] = self.motion_data_np[i, 6]  # u_right_t1

                # The output ground truth is the same as input, but offset by 1 icp hit
                self.data_y[_id-1, 0] = self.data_x[_id, 0] # roll
                self.data_y[_id-1, 1] = self.data_x[_id, 1]  # body v_x
                self.data_y[_id-1, 2] = self.data_x[_id, 2] # body v_y
                self.data_y[_id-1, 3] = self.data_x[_id, 3] # yaw rate
